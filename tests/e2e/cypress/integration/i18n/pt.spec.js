...
context('i18n | PT', () => {
    ...
    it('Should have PT sentences in Head component', () => {
      cy.fixture('i18nHead').then((translations) => {
        cy.get('head title').should('contain', translations.pt.title);
        cy.get('head meta[name="application-name"]').should('have.attr', 'content', translations.pt.appname);
        cy.get('head meta[name="description"]').should('have.attr', 'content', translations.pt.description);
        cy.get('head meta[name="keywords"]').should('have.attr', 'content', translations.pt.keywords);
      });
    });
    ...
    it('Should have PT sentences in Header component', () => {
      cy.fixture('selectors').then((object) => {
        cy.fixture('i18nPT').then((translations) => {
